select  concat('Кампания № ', ads_campaign) ads_campaign, round(avg(avg_check), 2) avg_check
from    (
        select  paying_user_id, min(ads_campaign) ads_campaign, round(sum(price) / count(distinct order_id), 2) avg_check
        from    (
                select  order_id, paying_user_id, ads_campaign, unnest(product_ids) product_id
                ...
                where   ads_campaign in (1, 2) and
                        creation_time >= '2022-09-01' and creation_time < '2022-09-08'
                ) t2 left join
                products using (product_id)
        group by paying_user_id
        ) t3
