select  concat('Кампания № ', ads_campaign) ads_campaign,
        ...
        round(count(user_id) / max(count(user_id)) over (partition by ads_campaign, start_date)::decimal, 2) retention
from    (
        select  distinct user_id, ads_campaign, start_date, time::date date
        from    (
                select  user_id,
                        case
                ...
                from    user_actions
                where   time >= '2022-09-01' and time < '2022-09-08'
                group by user_id        
                ) t1 left join
