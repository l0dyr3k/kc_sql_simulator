select  'Кампания № 1' ads_campaign,
        round(250000::decimal / (
                                select  count(distinct user_id) paying_users
                                from    user_actions
                                where   order_id not in (select order_id from user_actions where action = 'cancel_order') and
...
union
select  'Кампания № 2' ads_campaign,
        round(250000::decimal / (
                                select  count(distinct user_id) paying_users
                                from    user_actions
                                where   order_id not in (select order_id from user_actions where action = 'cancel_order') and
