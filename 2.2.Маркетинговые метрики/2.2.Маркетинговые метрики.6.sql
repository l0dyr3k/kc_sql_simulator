select  concat('Кампания № ', ads_campaign) ads_campaign,
        concat('Day ', date - '2022-09-01'::date) as day,
        round(sum(sum(price) / min(paying_users)) over (partition by ads_campaign order by date - '2022-09-01'::date), 2) cumulative_arppu,
        round(min(cac), 2) cac
from    (
        select  ads_campaign,
                count(distinct paying_user_id) paying_users,
                round(250000 / count(distinct paying_user_id)::decimal, 2) cac
        from    (
                select  user_id paying_user_id,
                        case
                        when user_id in (8804, 9828, 9524, 9667, 9165, 10013, 9625, 8879, 9145, 8657, 8706, 9476, 9813, 
                                         ...
                                         9491, 8739, 8770, 9069, 9278, 9831, 9291, 9089, 8976, 9611, 10082, 8673, 9113, 10051) then 1
                        when user_id in (9752, 9510, 8893, 9196, 10038, 9839, 9820, 9064, 9930, 9529, 9267, 9161, 9231, 
                                         ...
                                         9327, 9790, 8719, 9706, 9289, 9047, 9495, 9558, 8650, 9784, 8935, 9764, 8712) then 2
                        else 0
                        end ads_campaign
                from    user_actions
        ...
        from    user_actions right join
                orders using (order_id)
        where   order_id not in (select order_id from user_actions where action = 'cancel_order')        
        ) t3 using (ads_campaign) join
        products using(product_id)
where ads_campaign in (1, 2)
group by ads_campaign, date, day
